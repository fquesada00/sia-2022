...
def plot_linear_separation(input_dataset, expected_output, weights):
    ...
    for data_number, data in enumerate(input_dataset):
        plt.plot(data[0], data[1], marker='o', color=f'{ "blue" if expected_output[data_number][0] == 1 else "red" }')
    
    legends = [f"Input #{index} [{input_data[0]}, {input_data[1]}]" for index, input_data in enumerate(input_dataset)]
