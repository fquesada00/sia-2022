...
def plot_ej_2_linear_with_batches(number_of_epochs=1, iterations_per_network=1, batches=[10, 20, 50, 100, 200]):
    ...
    plot_epoch_vs_error(epochs, mean_errors, stdev_errors, legends, header=header, values=values, file_path=TRAIN_ERROR_BY_EPOCH_CSV_FILE_PATH)
...
def plot_epoch_vs_error(epochs: dict, mean_errors_by_set, error_stdevs_by_set, legends, header="", values=[], file_path=None):
    """
    ...
    with open(file_path, "w") as csv_file:
        ...
        for i in range(len(mean_errors_by_set)):
            ...
            csv_file.write(f"{values[i]},{','.join([f'{round(mean_error, 2)}' for mean_error in mean_errors])}\n")
