...
class NeuralNetwork:
    def __init__(self, hidden_sizes: list, input_size: int, output_size: int, learning_rate: float, bias: float, activation_function_str: str, batch_size=1, output_file_name: str = None,beta:float = 1):
        ...
        self.initialize_layers(input_size, output_size, hidden_sizes,beta)
    ...
    def test(self, input_dataset, expected_output):
        ...
        for input_data in input_dataset:
            ...
            predictions = np.append(predictions, prediction).reshape(-1, len(prediction))
